...
mod tetris
{
    ...
    enum Shape
    {
        ShapeO, ShapeI, ShapeS, 
        ShapeZ, ShapeL, ShapeJ, ShapeT, 
    }
    ...
    impl Mino 
    {
        ...
        fn new() -> Mino 
        {
            ...
            {
                piece.cords[i][0] = 
                    (cent_x + CORD_LIST[piece.shape as usize][i][0]) as u8;
                
                piece.cords[i][1] = 
                    (cent_y + CORD_LIST[piece.shape as usize][i][1]) as u8;
            }
    ...
    impl Game
    {
        ...
        fn can_rotate(&self) -> bool
        {
            ...
            {	
                ...
                if new_x >= B_LEN as i16 || new_x < 0 || new_y < 0 ||
        ...
        fn can_shift(&self, dir : bool) -> bool 
        {
            ...
            if next_x >= B_LEN as i8 || next_x < 0 ||
        ...
        fn update_preview(&mut self)
        {
            ...
            if self.has_held
            {
                ...
                for i in 0..CORD_NUM 
                {
                    let x = (cent_x as i8 + CORD_LIST[self.held_mino.shape as usize][i][0]) as usize;
                    let y = (cent_y as i8 + CORD_LIST[self.held_mino.shape as usize][i][1]) as usize;
            ...
            {
                self.next_board[cent_x][cent_y as usize] = COLOR_LIST[self.next_minos[j].shape as usize]; 

                for i in 0..CORD_NUM 
                {
                    let x = (cent_x as i8 + CORD_LIST[self.next_minos[j].shape as usize][i][0]) as usize;
                    let y = (cent_y as i8 + CORD_LIST[self.next_minos[j].shape as usize][i][1]) as usize;
        ...
        fn line_up(&mut self)
        {
            ...
            if self.lines / (3 * (self.level + 1)) as u16 != 0 
            {
                self.level += 1;
                self.calc_speed();
            }
